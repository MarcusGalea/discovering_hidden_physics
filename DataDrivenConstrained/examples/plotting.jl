function plot_interpolation(sol, interval; interpolation = InterpolationMethod(CubicSpline), variable_names = hcat(["x$i" for i in 1:size(sol.u[1], 1)]...))
...
function plot_data(df, dfsol, u; ddsolconstrained = nothing, t_interval = collect(1:1:size(dfsol, 1)))
    ...
    for i in 1:num_states
        variable = string(u[i])
        plot!(p, df.t[t_interval], df[t_interval,variable], label = variable, color = colors[1], subplot = i, linewidth = linewidth, alpha = opacity1)
        ...
        if !isnothing(ddsolconstrained)
            plot!(p, ddsolconstrained.t[t_interval], ddsolconstrained[i, t_interval], label = variable * "_constrained", linestyle = :dot, color = colors[3], subplot = i, linewidth = linewidth, alpha = opacity2)
        end
    end
