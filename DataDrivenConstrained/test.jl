...
ddprob2 = DataDrivenProblem(sol, use_interpolations = true)
X = hcat(sol.u...) 
...
ddprob = ContinuousDataDrivenProblem(X, sol.t, DX) 
#plot derivative interpolation vs true derivative#log scale
...
#savefig in plots folder
savefig(p1, "plots/residual_true_vs_interpolated_derivatives.png")
...
ddprob = DataDrivenProblem(sol, use_interpolations = false)
opt = DataDrivenConstrained.ConstrainedSTLSQ(λ, 0.0, Equation[], hcat(Ξ))
solbasis, cache = solve(ddprob, basis, opt, options = DataDrivenCommonOptions())
...
optconstrained = DataDrivenConstrained.ConstrainedSTLSQ(λ,0.0, constraints, hcat(Ξ))
solbasisconstrained, cache2 = solve(ddprob, basis, optconstrained, options = DataDrivenCommonOptions())
