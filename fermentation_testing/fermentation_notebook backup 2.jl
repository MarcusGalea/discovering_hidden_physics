...
begin
  using Revise, DifferentialEquations, ModelingToolkit, Plots, PlutoUI, DataFrames, Random
...
begin
  @parameters glucose_0 μ_DT μ_L μ_x0 k_x μ_SD0 μ_glucose_max k_glucose μ_metabolite_max k_metabolite μ_DY μ_AB Y_EA
  @parameters ΔH_total T_C0 heat_transfer optimal_temperature sd_temp O_solubility kla
  @variables X_latent(t) X_active(t) X_dead(t)
  @variables glucose(t) metabolite(t) ethyl_acetate(t) diacetyl(t)
  @variables jacket_temperature(t) fermentor_temperature(t) coolant_rate(t)
  @variables F_feed(t) feed_on(t) μ_x(t) μ_SD(t) μ_glucose(t) μ_metabolite(t) X_total(t)
end
...
@named fermentation_sys = ODESystem([
  ...
  μ_x ~ μ_x0 * (glucose / (k_x)) * exp(-(fermentor_temperature - optimal_temperature)^2 / (2 * sd_temp^2)),
...
begin
    ...
    idx_dict = Dict(zip(string.(unknowns(sys)), eachindex(unknowns(sys))))

    condition(u, t, integrator) = u[idx_dict["fermentor_temperature(t)"]] > 35.0
    affect!(integrator) = (integrator.u[idx_dict["coolant_rate(t)"]] = coolant_rate_activation_slider)
    cb = DiscreteCallback(condition, affect!)

    condition_stop(u, t, integrator) = u[idx_dict["fermentor_temperature(t)"]] < 10.0
    affect_stop!(integrator) = (integrator.u[idx_dict["coolant_rate(t)"]] = 0.0)
    cb_stop = DiscreteCallback(condition_stop, affect_stop!)

    condition_feed(u, t, integrator) = u[idx_dict["glucose(t)"]] < 100.0
    affect_feed!(integrator) = (integrator.u[idx_dict["feed_on(t)"]] = 1.0)
    cb_feed = DiscreteCallback(condition_feed, affect_feed!)

    condition_feed_stop(u, t, integrator) = t >= 100.0
    affect_feed_stop!(integrator) = (integrator.u[idx_dict["feed_on(t)"]] = 0.0)
    ...
    u0 = [
        ...
        X_active => active0,
        X_dead => dead0,
        glucose => glucose_0_slider,
        metabolite => metabolite0,
        ethyl_acetate => ethyl_acetate0,
        diacetyl => diacetyl0,
        jacket_temperature => jacket_temperature0,
        fermentor_temperature => fermentor_temperature0,
        coolant_rate => 0.0,
        F_feed => 0.0,
        feed_on => 0.0,
    ]
    ...
    p = [
        glucose_0 => glucose_0_slider,
        μ_DT => μ_DT_slider,
        μ_L => μ_L_slider,
        μ_x0 => μ_x0_slider,
        k_x => k_x_slider,
        μ_SD0 => μ_SD0_slider,
        μ_glucose_max => μ_glucose_max_slider,
        k_glucose => k_glucose_slider,
        μ_metabolite_max => μ_metabolite_max_slider,
        k_metabolite => k_metabolite_slider,
        μ_DY => μ_DY_slider,
        μ_AB => μ_AB_slider,
        Y_EA => Y_EA_slider,
        ΔH_total => ΔH_total_slider,
        T_C0 => T_C0_slider,
        heat_transfer => heat_transfer_slider,
        optimal_temperature => optimal_temperature_slider,
        sd_temp => sd_temp_slider,
        O_solubility => O_solubility_slider,
        kla => kla_slider,
    ]

    prob = ODEProblem(sys, u0, (0.0, 150.0), p, callback = cb_combined)
...
manifest_format = "2.0"
project_hash = "761e80c781357c1a38923fb4dac60b767404e28e"
...
git-tree-sha1 = "563bb87e9902f3adffe2568e2af6317d6675dbcd"
...
git-tree-sha1 = "bd1451e3684e412792a1649cdcc20d008f0d5400"
...
git-tree-sha1 = "7e6a48b479227bdbc372ea7f4affb04c458778e2"
...
git-tree-sha1 = "0ff40d7b68ab73909648974b825d493ddfad250c"
...
git-tree-sha1 = "77ed45e89623c8e468677f7dfec5b51abb5c311a"
...
git-tree-sha1 = "418f4d81ddcd262a89da8fb9f3524a29166c5403"
