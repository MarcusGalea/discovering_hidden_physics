...
@unpack E, S, ES, P, y = rn
obs = Dict("y" => y)#"E" => E, "S" => S, "ES" => ES, 
u0map = Dict([E => 10.0, S => 1.0, ES => 0.0, P => 0.0])
ic_vals = Dict(["cond$i" => Dict([var => ic[j] for (j, var) in enumerate(unknowns(rn))]) for (i, ic) in enumerate(initial_conditions[1:n_initial_conditions])])
