...
trainpeprob = HybridPEProblem(hmodel, obs, train_measurements_exp_2, u0map; random_sampling_percentage = random_sampling_percentage,
...
# equilibrium_prey = mean(train_measurements_exp_2[train_measurements_exp_2.obs_id .== "prey_o", :].measurement)
# equilibrium_predator = mean(train_measurements_exp_2[train_measurements_exp_2.obs_id .== "predator_o", :].measurement)
...
opt_sol = Optimization.solve(remake(opt_prob; u0 = initp_samples), ProgressivePolyOpt(lr = 1e-3, n_partitions = 10), EnsembleSerial(),
                            trajectories = 1,
                                maxiters = 1000,
                                maxiter_BFGS = 300,
